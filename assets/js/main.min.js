...
// addComent from Wordpress
var addComment={moveForm:function(a,b,c,d){var e,element,style,cssHidden,t=this,comm=t.I(a),respond=t.I(c),respondhead=t.I(c+'-head'),cancel=t.I('cancel-comment-reply-link'),parent=t.I('comment-replying-to'),post=t.I('comment-post-slug'),commentForm=respond.getElementsByTagName('form')[0];if(!comm||!respond||!cancel||!parent||!commentForm){return}t.respondId=c;d=d||false;if(!t.I('sm-temp-form-div')){e=document.createElement('div');e.id='sm-temp-form-div';e.style.display='none';respondhead_borderTop=respondhead.style.paddingTop;respondhead_paddingTop=respondhead.style.paddingTop;respondhead.style.borderTop='none';respondhead.style.paddingTop=0;respond.parentNode.insertBefore(e,respond)}comm.parentNode.insertBefore(respond,comm.nextSibling);if(post&&d){post.value=d}parent.value=b;cancel.style.display='';cancel.onclick=function(){var t=addComment,temp=t.I('sm-temp-form-div'),respond=t.I(t.respondId);if(!temp||!respond){return}t.I('comment-replying-to').value=null;respondhead.style.borderTop=respondhead_borderTop;respondhead.style.paddingTop=respondhead_paddingTop;temp.parentNode.insertBefore(respond,temp);temp.parentNode.removeChild(temp);this.style.display='none';this.onclick=null;return false};try{for(var i=0;i<commentForm.elements.length;i++){element=commentForm.elements[i];cssHidden=false;if('getComputedStyle'in window){style=window.getComputedStyle(element)}else if(document.documentElement.currentStyle){style=element.currentStyle}if((element.offsetWidth<=0&&element.offsetHeight<=0)||style.visibility==='hidden'){cssHidden=true}if('hidden'===element.type||element.disabled||cssHidden){continue}element.focus();break}}catch(er){}return false},I:function(a){return document.getElementById(a)}};
